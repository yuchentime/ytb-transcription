...
- 默认分段阈值为 `8000 token`（更保守，优先保证长文本稳定性）。
...
- 合并翻译结果时，按顺序直接拼接（`\n` 连接）。
- 当检测到“翻译截断/大段未翻译”（如 `finish_reason=length` 或目标语脚本占比异常）时，会判定分段失败，阻断后续 TTS。
- 若首次触发上述异常且当前仅 1 个翻译分段，会自动降阈值到 `3000 token` 重切并重试 1 次；仍失败则终止任务并报错。
...
- `translateSplitThresholdTokens` 一般保持默认（`8000`）即可。
- 若仍偶发截断，可继续下调到 `3000 ~ 6000`。
